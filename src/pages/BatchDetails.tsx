...
export const BatchDetails: React.FC = () => {
  ...
  const batch = BATCHES_DATA.find(b => b.id === id);
  ...
  if (!batch) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        ...
        <Button onClick={() => navigate('/batches')}>Back to All Batches</Button>
      </div>
    );
  }
      ...
      <div className="bg-gray-50 border-b border-gray-100">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <Link to="/batches" className="hover:text-[var(--color-primary)] transition-colors flex items-center gap-1">
              <ArrowLeft size={14} /> Back to Batches
            </Link>
            <span>/</span>
            <span className="text-gray-900 font-medium truncate">{batch.title}</span>
          </div>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4 mb-10">
                <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-gray-100 shadow-sm">
                  ...
                  <span className="text-sm font-bold text-gray-700">{batch.duration}</span>
                </div>
                <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-gray-100 shadow-sm">
                  <BookCheck size={18} className="text-purple-500" />
                  <span className="text-sm font-bold text-gray-700">{batch.medium}</span>
                </div>
              </div>

              <div className="flex items-center gap-6">
                <div>
                  <span className="text-sm text-gray-400 line-through block">₹{batch.originalPrice.toLocaleString()}</span>
                  <span className="text-4xl font-black text-gray-900">₹{batch.discountedPrice.toLocaleString()}</span>
                </div>
            ...
            <div className="space-y-12">
              ...
              <div className="bg-gray-900 text-white p-8 rounded-[2.5rem] relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <p className="text-2xl font-bold mb-6">{batch.details.validity}</p>
