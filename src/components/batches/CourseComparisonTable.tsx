    ...
    <section className="py-20 bg-gray-50 overflow-hidden">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-black text-gray-900 mb-4">Compare Our Batches</h2>
          ...
          <table className="w-full min-w-[1000px] border-separate border-spacing-y-4">
            ...
            <tbody>
              {BATCHES_DATA.map((batch) => (
                <tr key={batch.id} className="bg-white group hover:shadow-xl hover:shadow-blue-500/5 transition-all">
                  ...
                  <td className="px-8 py-6 border-y border-gray-100 text-gray-600 font-medium">{batch.target}</td>
                  <td className="px-8 py-6 border-y border-gray-100 text-gray-600">{batch.medium}</td>
                  <td className="px-8 py-6 border-y border-gray-100 text-gray-600">{batch.duration}</td>
                  <td className="px-8 py-6 border-y border-gray-100">
                    <span className="font-black text-gray-900">₹{batch.discountedPrice.toLocaleString()}</span>
                  </td>
                  <td className="px-8 py-6 border-y border-gray-100">
                    <span className="text-xs text-gray-500 line-clamp-1">{batch.details.whoIsItFor[0]}</span>
                  </td>
        ...
        <div className="md:hidden space-y-4">
          {BATCHES_DATA.slice(0, 3).map((batch) => (
            <div key={batch.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-900 mb-4">{batch.title}</h3>
              <div className="grid grid-cols-2 gap-4 text-sm mb-6">
                <div>
                  <p className="text-gray-400 text-xs mb-1 uppercase font-black">Target</p>
                  <p className="text-gray-700 font-bold">{batch.target}</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs mb-1 uppercase font-black">Price</p>
                  <p className="text-gray-900 font-black">₹{batch.discountedPrice.toLocaleString()}</p>
                </div>
              </div>
